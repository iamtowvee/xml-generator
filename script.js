document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderElement(element, container, level = 0) {
        ...
        if (element.isSection) {
            contentHTML += `
                <span class="hint" style="margin-left: 10px;">(раздел)</span>
            `;
        } else {
            contentHTML += `
    ...
    function generateXml(element, indent = 0) {
        ...
        if (element.isSection) {
            // Раздел - это тег, который может содержать другие теги
            let xml = '';
    ...
    function formatXml(xml) {
        ...
        for (let line of lines) {
            ...
            if (trimmed || line.includes('<?xml') || line.includes('<') && line.includes('>')) {
                result.push(line);
            }
        }
    ...
    if (state.elements.length === 0) {
        const root = createElement('making', '', true);
        createElement('tag', 'check', false, root.id);
    }
});
